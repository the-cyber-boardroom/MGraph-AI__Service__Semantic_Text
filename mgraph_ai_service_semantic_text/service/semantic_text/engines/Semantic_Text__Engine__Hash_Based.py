...
class Semantic_Text__Engine__Hash_Based(Semantic_Text__Engine):                # Deterministic hash-based classification engine
    ...
    def classify_text(self, text: Safe_Str__Text) -> Schema__Semantic_Text__Classification:
        classification_value = self.hash_based_classification(text)
    ...
    @type_safe
    def hash_based_classification(self, text: Safe_Str__Text) -> Safe_Float__Text__Classification:  # Combine text + criteria for unique rating per criterion
        combined  = f"{text}_{self.classification__criteria.value}"
